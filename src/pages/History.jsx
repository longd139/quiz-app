      ...
      {(session.questions || []).map((q, i) => {
        ...
        return (
          <div key={i} className={`bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-none border border-slate-200 dark:border-slate-700 p-4 mb-3 ${ok ? 'border-l-4 border-l-success-600' : 'border-l-4 border-l-danger-600'}`}>
            ...
            {q.explanation && (
              <div className="mt-3 p-3 bg-primary-50 dark:bg-primary-700/20 rounded-lg text-[0.8rem] leading-relaxed"><strong>Giải thích:</strong> <span dangerouslySetInnerHTML={{ __html: renderMd(q.explanation) }} /></div>
            )}
          </div>
        );
      })}
